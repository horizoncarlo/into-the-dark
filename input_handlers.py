...
class MainGameEventHandler(EventHandler):
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        # elif key == tcod.event.KeySym.COMMA and modifier & (tcod.event.Modifier.LSHIFT | tcod.event.Modifier.RSHIFT):
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_item_selected(self, item: Item) -> Optional[ActionOrHandler]:
        ...
        # TTODO On selecting an item for use, or after targetting, we should stop the action.
        # For example targetted spells can be cast infinitely as they never lose the target step
        raise NotImplementedError()
...
class LevelUpEventHandler(AskUserEventHandler):
    ...
    def on_render(
        ...
        console.print(
            x=x + 1,
            y=4,
            string=f"a) Heart (+10 HP, from {self.engine.player.fighter.max_hp})",
        )
        console.print(
            x=x + 1,
            y=5,
...
class CharacterScreenEventHandler(AskUserEventHandler):
    ...
    def on_render(
        ...
        console.print(
            x=x + 1,
            y=y + 3,
            string=f" Next:   {self.engine.player.level.experience_to_next_level}",
        )
